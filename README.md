# 🛍️ Full-Stack E-Commerce Mobile App

A **production-ready E-Commerce platform** built with **React Native (Expo)**, **Node.js**, and a powerful **Admin Dashboard**, featuring secure authentication, payments, background jobs, analytics, and modern DevOps practices.

---

## ✨ Highlights

- 📱 Fully Functional E-Commerce **Mobile App** (React Native + Expo)
- 🔐 Secure Authentication with **Clerk** (Google & Apple Sign-In)
- 🛒 Cart, Favorites, Checkout & Orders Flow
- 💳 **Stripe-Powered Payments**
- 🗺️ Address Management System
- 🏪 **Admin Dashboard**
  - Products
  - Orders
  - Customers
  - Live Statistics
- ⚙️ Complete **REST API** (Node.js + Express)
- 🛂 **Role-Based Access Control** (Admin-only routes)
- 📦 Background Jobs with **Inngest**
- 🧭 Live Analytics Dashboard
- 🛠️ Product Management (CRUD, images, pricing)
- 📦 Order Management System
- 👥 Customer Management Page
- 🛡️ **Sentry Integration** for monitoring & error tracking
- 🚀 Deployment on **Sevalla** (API + Admin Dashboard)
- 🖼️ Product Image Slider
- ⚡ Data Fetching & Caching with **TanStack Query**
- 🧰 End-to-End **Git & GitHub Workflow**
- 🤖 **CodeRabbit PR Analysis** (security, quality, optimization)

---

## 🧪 Environment Variables Setup

### 🟦 Backend (`/backend/.env`)

```env
NODE_ENV=development
PORT=3000

DB_URL=<YOUR_DB_URL>

CLERK_PUBLISHABLE_KEY=<YOUR_CLERK_PUBLISHABLE_KEY>
CLERK_SECRET_KEY=<YOUR_CLERK_SECRET_KEY>

INNGEST_SIGNING_KEY=<YOUR_INNGEST_SIGNING_KEY>

CLOUDINARY_API_KEY=<YOUR_CLOUDINARY_API_KEY>
CLOUDINARY_API_SECRET=<YOUR_CLOUDINARY_API_SECRET>
CLOUDINARY_CLOUD_NAME=<YOUR_CLOUDINARY_CLOUD_NAME>

ADMIN_EMAIL=<YOUR_ADMIN_EMAIL>

CLIENT_URL=http://localhost:5173

STRIPE_PUBLISHABLE_KEY=<YOUR_STRIPE_PUBLISHABLE_KEY>
STRIPE_SECRET_KEY=<YOUR_STRIPE_SECRET_KEY>
STRIPE_WEBHOOK_SECRET=<YOUR_STRIPE_WEBHOOK_SECRET>
```

##🟩 Admin Dashboard (/admin/.env)

```env
VITE_CLERK_PUBLISHABLE_KEY=<YOUR_CLERK_PUBLISHABLE_KEY>
VITE_API_URL=http://localhost:5000/api
```

##🟧 Mobile App (/mobile/.env)

```env
EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY=<YOUR_CLERK_PUBLISHABLE_KEY>
EXPO_PUBLIC_STRIPE_PUBLISHABLE_KEY=<YOUR_STRIPE_PUBLISHABLE_KEY>

SENTRY_AUTH_TOKEN=<YOUR_SENTRY_DSN>
```

#🚀 Running the Project Locally

##🔧 Run the Backend

```bash
cd backend
npm install
npm run dev
```
##🔧 Run the Admin Dashboard

```bash
cd admin
npm install
npm run dev
```

##🔧 Run the Mobile App

```bash
cd mobile
npm install
npx expo start

```

##📱 Scan the QR code using the Expo Go app on your phone.
